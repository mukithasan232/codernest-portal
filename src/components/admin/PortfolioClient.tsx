  ...
  return (
    <div className="w-full h-full animate-in fade-in duration-500">
      ...
      <div className="flex items-center gap-3 text-cyan-400 mb-8">
        ...
        <h1 className="text-3xl font-extrabold text-white tracking-tight">Portfolio Manager</h1>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-8">
        {/* Upload Form */}
        <div className="lg:col-span-1 bg-white/[0.02] backdrop-blur-xl border border-white/10 rounded-3xl p-6 md:p-8 h-fit">
          <h2 className="text-xl font-bold text-white mb-2">New Processing Job</h2>
          <p className="text-sm text-slate-400 mb-6">Upload a raw image to automatically process it through the Sharp enhancement pipeline.</p>
          ...
          <div className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-slate-400 mb-2">Raw Image</label>
              <div 
                className={`w-full h-48 border-2 border-dashed ${previewUrl ? 'border-blue-500/50 bg-blue-500/5' : 'border-white/20 bg-black/20'} rounded-2xl flex flex-col items-center justify-center hover:bg-white/5 hover:border-blue-500/50 transition-all cursor-pointer overflow-hidden relative group`}
              ...
              >
                {previewUrl ? (
                  ...
                  <div className="text-center p-4">
                    ...
                    <span className="text-sm text-slate-300 font-medium block">Click to upload raw image</span>
        ...
        <div className="lg:col-span-2">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            {images.length === 0 ? (
              <div className="sm:col-span-2 py-20 text-center bg-white/[0.02] border border-white/5 rounded-3xl">
                <ImageIcon className="w-12 h-12 text-slate-600 mx-auto mb-4" />
                <h3 className="text-lg font-medium text-slate-300 mb-1">No images processed yet</h3>
                <p className="text-slate-500 text-sm">Upload your first image to test the pipeline.</p>
              </div>
            ) : (
              images.map(img => (
                <div key={img.id} className="bg-white/[0.02] border border-white/10 rounded-2xl overflow-hidden group">
                  <div className="relative h-48 w-full bg-black/50 border-b border-white/10 flex">
                    {/* Fake Slider Preview */}
                  ...
                  <div className="p-4 flex items-center justify-between">
                    <div>
                      <h3 className="font-semibold text-white truncate max-w-[200px]">{img.title}</h3>
